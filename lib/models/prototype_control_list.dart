...
import 'package:onboarding/components/controls/generic/generic_divider.dart';
...
class PrototypeControlList with ChangeNotifier {
  final List<ControlDisplayItem> _controls = [
    ...
    ControlDisplayItem(
      ...
      category: "Generic",
      name: "Divider",
      createWidget: (key) => GenericDivider(key: key),
      createForm: (key) => SmallHeadlineForm(key: key),
      displayWidget: const GenericDivider(key: ValueKey<int>(-3)),
    ),
    ControlDisplayItem(
      id: -4,
      ...
      displayWidget: const PersonName(key: ValueKey<int>(-4)),
    ),
    ControlDisplayItem(
      id: -5,
      category: "Medical",
      ...
      createForm: (key) => GenericTextForm(key: key),
      displayWidget: const ImageCapture(key: ValueKey<int>(-5)),
    ),
  ];
